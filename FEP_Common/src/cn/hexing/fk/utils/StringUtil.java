...
public class StringUtil {
	...
	public static String PadLeftForStr(String str, int strLength,String PadStr) {
		...
		if (strLen < strLength) {
			while (strLen < strLength) {
				...
				sb.append(PadStr).append(str);//左补字符
				str = sb.toString();
				strLen = str.length();
			}
		}
	...
	/**
	 * 将obis转换成16进制
	 * @param classId 2个字节，前面补上"00"
	 * @param obis
	 * @param attr 属性id	 2个字节，后面补上"00"
	 * @return
	 */
	...
	/**
	 * 分批加载左边树里的内容 当加载数据>size的时候 
	 * count 左边树加载分割成多少块 counts 每次往LIst加载的个数 size 每块分割有多少数据
	 * 控制类不导出Excel的时候用
	 */
