 ...
 * 
 */
package cn.hexing.fk.common.threadpool;
...
import cn.hexing.fk.common.events.event.KillThreadEvent;
...
public class ThreadPool implements ITimerFunctor{
	...
	public boolean start(){
		...
		if( log.isDebugEnabled() )
			log.debug("Thread pool【"+name+"】Start Success。,size="+minSize);//log.debug("线程池【"+name+"】启动成功。,size="+minSize);
		return true;
	}
	...
	public void stop(){
		...
		if( log.isDebugEnabled() )
			log.debug("Thread pool【"+name+"】stop。,Dead thread size="+works.size());//log.debug("线程池【"+name+"】停止。,僵死线程数="+works.size());
		works.clear();
		state = State.STOPPED;
	}
	...
	private void forkThreads(int delta) {
		...
		if (delta > 0) {
			...
			if( log.isDebugEnabled() && 1 ==  delta )
				log.debug("Adjustment thread pool size(+1)");//log.debug("调整线程池大小(+1)");
			for (; delta > 0; delta--) {
				new WorkThread();
			}
		} else {
		...
		public void run() {
			...
			while( !ThreadPool.this.state.isStopping() && !ThreadPool.this.state.isStopped() ){
				try{
					...
					//线程被中断。检查是否需要关闭
					log.warn("event handle exp:"+exp.getLocalizedMessage(),exp);
					continue;
				}
			}
			synchronized(works){
			...
			log.info("Thread pool work thread exit:"+this.getName());//log.info("线程池的工作线程退出:"+this.getName());
		}
...
//				this.interrupt();
		...
		public String toString(){
			...
			if( busy ){
				...
				sb.append(busyStatus).append(",stack trace=");
				EventHandleTimeoutAlarm ev = new EventHandleTimeoutAlarm(currentEvent);
				ev.setBeginTime(beginTime);
				ev.setEndTime(endTime);
				ev.setStackTraceElement(this.getStackTrace());
				sb.append(ev.toString());
				busyStatus = sb.toString();
			}
