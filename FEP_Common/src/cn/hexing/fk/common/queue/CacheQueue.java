 ...
 * 基于优先级的缓存队列。
 */
package cn.hexing.fk.common.queue;
...
public class CacheQueue {
	...
	public void asyncSaveQueue(){
		...
		synchronized(tempQueue){
			...
			t.start();
			log.info("asyncSaveQueue,size="+tempQueue.size());
		}
	}
	...
	private boolean _loadFromFile(){	
		long interval=System.currentTimeMillis()-this.lastCacheRead;
		synchronized(timeoutLock){			
			if( interval < this.minInterval ){
				log.debug("interval<minInterval,interval="+interval+",minInterval="+minInterval);
				return false;
			}
//			else
//				log.info("can read cache file,key="+key);
		...
		synchronized(fileLock){		
			...
			if( null == filename ){
				log.info("can not find Read Cache File Name");
				return false;
			}
			if( log.isInfoEnabled() )
				log.info("start read cache file(开始加载缓存文件):"+filename);
			try{
				...
				while( null != (serial=raf.readLine()) ){
					if( serial.length()<26 ){
						log.warn("读缓存错误：读取无效内容,serial："+serial);
						continue;
					}
					//注意，目前只支持浙江规约格式消息
...
//			log.debug("filepath="+f.getPath()+",fname0="+fname0+":目录下无缓存文件。");
		return null;
	}
