...
public class PduParser
{
		...
		if (addressLength > 0)
		{
			...
			switch (PduUtils.extractAddressType(addressType))
			{
				case PduUtils.ADDRESS_TYPE_ALPHANUMERIC:
					...
					return stripAddressFieldPadding(addressLength, PduUtils.decode7bitEncoding(addressData));
				default:
	 ...
	 * GSM alphabet.
	 * 
	 * @param validSemiOctetNumber
	 *            the number of valid semi octets as given in the address length
	 *            field
	 * @param addressFieldString
	 *            the address string as per interpretation of whole field
	 * @return the actual address string without possibly existing characters
	 *         from interpreting fill bits
	 */
