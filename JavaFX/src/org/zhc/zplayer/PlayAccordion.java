...
public class PlayAccordion implements EventHandler<WindowEvent>,MenuListener{
   static ObjectProperty<MusicInfo> playIndex=
		  new SimpleObjectProperty<MusicInfo>(new MusicInfo(null,null,null));
  ...
  public void playMusic(){
	...
	playMusic(playIndex.get(),
		   findViewByGroup(groups.get(0)).getContentView().getItems().get(0));
	MusicInfo.myindex++;
   }
  ...
  public void playPrevMusic(){
	...
	if(MusicInfo.myindex==0){
	   ...
	   result=findViewByGroup(last.group).getContentView()
			  .getItems().get(MusicInfo.myindex-1);
	   MusicInfo.myindex--;
	  }
  ...
  public Map<String,List<MusicInfo>> getMusicData(){
	...
	for(MusicContainer mc:listmc){
	  if(mc.getListContainer()!=null)
	    result.put(mc.getMusicGroup(),mc.getListContainer().getView().getItems());	
	 }
  ...
  private void initAccordion(){
	...
	if(listmc.size()>0)
	  listmc.get(0).getView().setExpanded(true);
  ...
  void loadContextMenu(Set<String> groups){
	 ...
	 cm.setOnShown(this);
	 cm.setOnHidden(this);
	 GroupTitle.setMenuListener(this);
	 ...
	 move.getItems().addAll(new SeparatorMenuItem(),new MenuItem("   新建列表  \t",
		 ResourceManager.getViewOfClasspath("menu_add.png")));
	
	 cm.getItems().addAll(new MenuItem("   播放\t"),move,new MenuItem("   收藏到列表",
		  ResourceManager.getViewOfClasspath("favorite.png")),
		MenuItemBuilder.create()
		.text("   移除歌曲\t").onAction(moveHandler)
		.build(),new MenuItem("   删除歌曲\t",
		  ResourceManager.getViewOfClasspath("menu_del.png")),
		new MenuItem("   歌曲去重工具\t"),new SeparatorMenuItem(),
	    new MenuItem("   为歌曲匹配MV\t",
		  ResourceManager.getViewOfClasspath("menu_mv.png")),
		new SeparatorMenuItem(),MenuBuilder.create().text("   重命名\t")
		   .items(new MenuItem("   智能重命名\t"),new MenuItem("   手动重命名\t")).build(),
		MenuBuilder.create().text("   播放模式\t")
		   .items(new MenuItem("   单曲播放\t"),new MenuItem("   顺序播放\t",
				  ResourceManager.getViewOfClasspath("menu_check.png")),
			   new MenuItem("   单曲循环\t"),new MenuItem("   全部循环\t"),
			   new MenuItem("   随机播放\t"),new SeparatorMenuItem(),
			   new MenuItem("   自动切换列表\t",
				ResourceManager.getViewOfClasspath("menu_check.png"))).build(),
	    MenuItemBuilder.create()
	      .text("   歌曲排序\t").onAction(moveHandler)
	      .build(),new MenuItem("   打开文件所在目录\t"),new MenuItem("   导出列表文件"),
	    new MenuItem("   发送到移动设备\t",
	       ResourceManager.getViewOfClasspath("menu_send.png")),
	    new MenuItem("   歌曲信息\t"),new SeparatorMenuItem(),new MenuItem("   搜索相关歌曲\t"));
   }
   
   EventHandler<ActionEvent> getGroupHandler(){
	 if(group_menu_handler!=null)
   ...
   public void handle(WindowEvent event){
	 if(event.getEventType()==WindowEvent.WINDOW_SHOWN){
		MusicTips.contextmenushow=true;
	 }else if(event.getEventType()==WindowEvent.WINDOW_HIDDEN){
		MusicTips.contextmenushow=false;
	 }
    }
...
//  static class Index{
//	public String group;
//	//index为MusicInfo在它所处List的位置
//	public int index;
//	//index为MusicInfo的hash
//	public int id;
//	
//	public Index(){}
//	
//	public Index(String group,int index){
//	  this.group=group;
//	  this.index=index;
//	 }
//	
